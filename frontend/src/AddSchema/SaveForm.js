...
const SaveForm = ({ schema }) => {
  ...
  useEffect(() => {
    if (name.length === 0 && saveButton) setSaveButton(false);
    ...
    if (name.length > 0 && name.length < 50 && !saveButton) setSaveButton(true);
  ...
  const renderError = () => {
    ...
    return (
      <div className="border">
        <span className="mx-3 my-2" style={{ fontSize: 18, color: "#933636" }}>
          {errorMessage}
        </span>
      </div>
    );
  ...
  const renderInfo = () => {
    ...
    return (
      <div className="border">
        <span className="mx-3 my-2" style={{ fontSize: 18, color: "#3e2c6b" }}>
          {infoMessage}
        </span>
      </div>
    );
  ...
  return (
    <div>
      ...
      <SimpleCheck
        value={isNonLegal}
        setValue={setIsNonLegal}
        text={"Does this schema contain any illegal content?"}
        className="my-2"
      />
      {saveButton ? (
        <button onClick={() => publishSchema()} className="btn btn-success">
          Publish
        </button>
      ) : null}
    </div>
