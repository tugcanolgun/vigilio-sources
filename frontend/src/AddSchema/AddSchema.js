...
const AddSchema = () => {
  ...
  const isMandatoryResult = () => {
    ...
    if (
      ...
      results[0].sources[0].source === undefined
    )
      return false;
  ...
  const renderPage = () => {
    if (page === "addSchema") {
      return (
        <>
          <Link to={"/example"}>Check out the example</Link>
          <h1>Add a source</h1>
          <SimpleInput
            value={searchInput}
          ...
          {isMandatoryFilled() ? null : (
            <b className="mx-3">imdbId and source has to be filled</b>
          )}
          {loading ? (
            <div className="my-2">
              <h3>Loading...</h3>
