    ...
    <div className="container-fluid">
      <div className="container">
        <div>
          ...
          <button onClick={() => addField()} className="btn btn-success my-1">Add a field</button>
          {Object.keys(schemas).map((keyName, i) => (
            <FieldInput key={keyName} obj={schemas[keyName]} updateFunc={updateSchemas} removeFunc={removeSchemas}/>
          ))}
          {isEmpty(schemas) ? null : <button className="btn btn-success">Test</button>}
        </div>
      </div>
    </div>
  );
